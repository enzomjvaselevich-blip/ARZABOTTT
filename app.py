...
@app.route("/")
def home(): return "ErzaBot Full Live"
...
def webhook():
    ...
    try:
        ...
        # IA
            ...
            # Respuesta IA simple gratis
            try:
                r=requests.get(f"https://api.simsimi.net/v2/?text={prompt}&lc=es", timeout=10)
                ans=r.json().get('success', f"🤖 Erza IA: Recibí '{prompt}' - Esta función IA está en modo demo, conectala a OpenAI luego.")
